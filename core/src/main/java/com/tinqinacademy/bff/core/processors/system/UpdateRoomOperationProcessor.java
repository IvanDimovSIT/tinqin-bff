...
@Service
@Slf4j
public class UpdateRoomOperationProcessor extends BaseOperationProcessor implements BffUpdateRoomOperation {
    ...
    @Override
    public Either<Errors, BffUpdateRoomOutput> process(BffUpdateRoomInput input) {
        return Try.of(() -> {
                    log.info("Start process input:{}", input);
                    validate(input);

                    UpdateRoomOutput hotelOutput = hotelRestExport.updateRoom(input.getRoomId(),
                            conversionService.convert(input, UpdateRoomInput.class));

                    BffUpdateRoomOutput output = conversionService.convert(hotelOutput, BffUpdateRoomOutput.class);
